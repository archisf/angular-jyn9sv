import { Component, ContentChild, EventEmitter, Input, OnDestroy, Output, Renderer2, TemplateRef } from '@angular/core';
...
export class GridContextMenuComponent implements OnDestroy {
  ...
  @Input()
  public menuItems: any[] = ['Move Up', 'Move Down'];
  ...
  private onCellClick({ dataItem, type, originalEvent, columnIndex, rowIndex }): void {
    console.log('onCellClick', dataItem, type, originalEvent, columnIndex, rowIndex);
    if (type === 'contextmenu') {
      originalEvent.preventDefault();
      this.menuItems =
        columnIndex === 0
          ? ['צמצם', 'הרכב']
          : ['הוספת תפקיד', 'מחיקת תבנית', 'חיפוש משמרת בתאריך', 'חיפוש עובד בתאריך', 'נעילה', 'בטל נעילה'];
